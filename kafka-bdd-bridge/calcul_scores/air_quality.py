...
class CLIENT_AIR_QUALITY:
  ...
  def IAQ(self, co2_data, co_data, unit, pm_data, cov_data):
    ...
    co2_score = self.co2_score(co2_data)
    co2_coeff = 1 + int(co2_score)/10
    score += (co2_score * co2_coeff)

    co_score = self.co_score(co_data, unit)
    co_coeff = 1 + int(co_score)/10
    score += (co_score * co_coeff)

    pm_score = self.pm25_score(pm_data)
    pm_coeff = 1 + int(pm_score)/10
    score += (pm_score * pm_coeff)
  
    cov_score = self.cov_score(cov_data)
    cov_coeff = 1 + int(cov_score)/10
    score += (cov_score * cov_coeff)

    # print(f"co2 score: {co2_score}, coeff: {co2_coeff}")
    # print(f"co score: {co_score}, coeff: {co_coeff}")
    # print(f"pm score: {pm_score}, coeff: {pm_coeff}")
    # print(f"cov score: {cov_score}, coeff: {cov_coeff}")

    count = co2_coeff + co_coeff + pm_coeff + cov_coeff

    return round(score/count, 4)
    
  
  def old_IAQ(self, co2_data, co_data, unit, pm_data, cov_data):
    score = 0
